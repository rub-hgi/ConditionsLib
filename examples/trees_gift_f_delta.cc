...
std::vector<uint64_t> gift_sbox{0x1, 0xa, 0x4, 0xc, 0x6, 0xf, 0x3, 0x9,
                                0x2, 0xd, 0xb, 0x7, 0x5, 0x0, 0x8, 0xe};

std::vector<uint64_t> gift_inverse(16);

void init() {
  for (uint64_t i = 0; i < 16; ++i)
    gift_inverse[gift_sbox[i]] = i;
}

std::vector<uint64_t> prepare_f_i(uint64_t delta) {
  std::vector<uint64_t> result(16);
  for (uint64_t i = 0; i < 16;++i) {
    result[i] = gift_sbox[gift_inverse[i]^delta] ;
  }
  return result;
}
...
std::vector<uint64_t> prepare_f_i_plus_x(uint64_t delta) {
  std::vector<uint64_t> result(16);
  for (uint64_t i = 0; i < 16;++i) {
    result[i] = gift_sbox[gift_inverse[i]^delta] ^ i ;
  }
  return result;
}
std::vector<uint64_t> prepare_f_i_straight(uint64_t delta) {
  std::vector<uint64_t> result(16);
  for (uint64_t i = 0; i < 16;++i) {
    result[i] = gift_inverse[gift_sbox[i]^delta] ;
  }
  return result;
}
...
std::vector<uint64_t> prepare_f_i_plus_x_straight(uint64_t delta) {
  std::vector<uint64_t> result(16);
  for (uint64_t i = 0; i < 16;++i) {
    result[i] = gift_inverse[gift_sbox[i]^delta] ^ i ;
  }
  return result;
}
...
void analyse_old(std::ostream& os, std::vector<uint64_t> & sbox, int dim) {
  ...
  for (uint64_t component = 1; component < 16; ++component) {
    ...
    try {
      ...
      auto gs = BruteForceOptimization::StartSearch(cfun);
      auto ls = SboxTools::LinearStructures(cfun, 4);
      os << "Result: \n" << *gs << std::endl;
      os << "Costs: " << gs->associated_cost_ << std::endl;
...
void Analyse(std::ostream& os, std::vector<uint64_t> & sbox, int dim) {
  ...
  VectorialBooleanFunction fun(sbox, dim, dim);
    try {
      os << "-------------------\n";
      auto gs = ExhaustiveBF::StartSearch(fun);
      filter(gs);
      ...
      for (const auto &g: gs) {
        os << *g << "\n";
        os << "Costs: " << g->associated_cost_ << std::endl;
      }
    } catch(std::logic_error &e) {
...
int main() {
  ...
  std::string modifier = "";
  /*
   * modifier is added in front of the filename.
   * This is helpful when testing small changes in the code.
   * */
  const auto dim = 4u;
  for (uint64_t delta = 1; delta < 16; ++delta) {
    {
      std::vector<uint64_t> sbox = prepare_f_i_straight(delta);
      std::ofstream ffile(modifier+"straightF" + std::to_string(delta) + ".txt");
      analyse_old(ffile, sbox, dim);
    }
    {
      std::vector<uint64_t> sbox = prepare_f_i_plus_x_straight(delta);
      std::ofstream ffile(modifier+"straightF" + std::to_string(delta) + "PlusX.txt");
      analyse_old(ffile, sbox, dim);
    }
  }
  for (uint64_t delta = 1; delta < 16; ++delta) {
    {
      ...
      std::ofstream ffile(modifier+"f" + std::to_string(delta) + ".txt");
      analyse_old(ffile, sbox, dim);
    }
    {
      std::vector<uint64_t> sbox = prepare_f_i_plus_x(delta);
      std::ofstream ffile(modifier+"f" + std::to_string(delta) + "PlusX.txt");
      analyse_old(ffile, sbox, dim);
    }
  }
  for (uint64_t delta = 1; delta < 16; ++delta) {
    {
      std::vector<uint64_t> sbox = prepare_f_i_straight(delta);
      std::ofstream ffile(modifier+"allStraightF" + std::to_string(delta) + "_FOUR.txt");
      Analyse(ffile, sbox, dim);
    }
    {
      std::vector<uint64_t> sbox = prepare_f_i_plus_x_straight(delta);
      std::ofstream ffile(modifier+"allStraightF" + std::to_string(delta) + "PlusX_FOUR.txt");
      Analyse(ffile, sbox, dim);
    }
  }
  for (uint64_t delta = 1; delta < 16; ++delta) {
    {
      std::vector<uint64_t> sbox = prepare_f_i(delta);
      std::ofstream ffile(modifier+"allF" + std::to_string(delta) + "_FOUR.txt");
      Analyse(ffile, sbox, dim);
    }
    {
      std::vector<uint64_t> sbox = prepare_f_i_plus_x(delta);
      std::ofstream ffile(modifier+"allF" + std::to_string(delta) + "PlusX_FOUR.txt");
      Analyse(ffile, sbox, dim);
    }
  }
  return 0;
